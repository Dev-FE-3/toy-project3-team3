...
import { getLikeStatus, addLike, updateLikeActive, getLikeCountByPlaylist } from "@/api/like";
...
export const useLikeStatus = (userId?: number, playlistId?: number) => {
  ...
  useEffect(() => {
    const fetchLikeCount = async () => {
      ...
      const count = await getLikeCountByPlaylist(playlistId);
      setLikeCount(count);
    };
  ...
  const like = useMutation({
    mutationFn: addLike,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["likeStatus", userId, playlistId] });
    },
  });
  ...
  const toggleLike = useMutation({
    ...
    onSuccess: () => {
      queryClient.refetchQueries({ queryKey: ["likeStatus", userId, playlistId] });
    },
  });
